...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`SELECT b.id, b.booking_reference, b.appointment_date, b.appointment_time,
       ...
       WHERE b.user_id = $1
       ORDER BY b.appointment_date DESC`, [req.user.id]);
        res.json({
            success: true,
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const bookingResult = await client.query(`SELECT id FROM "Booking" WHERE id = $1 AND user_id = $2`, [id, req.user.id]);
       ...
       WHERE b.id = $1 AND b.user_id = $2`, [id, req.user.id]);
...
router.patch('/:id', async (req, res) => {
    ...
    try {
       ...
       WHERE id = $2
       RETURNING id, booking_reference, appointment_status, updated_at`, [appointment_status, id]);
        res.json({
            success: true,
...
//# sourceMappingURL=clientBookings.js.map