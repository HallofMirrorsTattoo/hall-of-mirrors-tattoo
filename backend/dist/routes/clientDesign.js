...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        // Create design idea
        const result = await client.query(`INSERT INTO "DesignIdea" (design_idea_id, user_id, booking_id, image_url, description, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
       RETURNING design_idea_id, image_url, description, created_at`, [uuidv4(), req.user.id, booking_id || null, image_url, description || null]);
        res.status(201).json({
            success: true,
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        await client.connect();
        const result = await client.query(`SELECT design_idea_id, booking_id, image_url, description, created_at
       FROM "DesignIdea"
       WHERE user_id = $1
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const designResult = await client.query(`SELECT design_idea_id FROM "DesignIdea" WHERE design_idea_id = $1 AND user_id = $2`, [id, req.user.id]);
...
//# sourceMappingURL=clientDesign.js.map