...
export default function ArtistDashboard() {
  ...
  // Flash state
  interface FlashSlotData {
    id: string; title: string; price_pence: number; image_url: string | null;
    is_available: boolean; claimed_by_name: string | null; claimed_by_email: string | null;
    claimed_by_phone: string | null; claimed_at: string | null;
  }
  interface FlashDayData {
    id: string; event_date: string; title: string; description: string | null;
    is_active: boolean; slots: FlashSlotData[];
  }
  const [flashDays, setFlashDays] = useState<FlashDayData[]>([]);
  ...
  const [newDaySubmitting, setNewDaySubmitting] = useState(false);
  ...
  const fetchFlashDays = async () => {
    ...
    setFlashError('');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load flash days');
      setFlashDays(data.flash_days || []);
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to load');
    } finally {
      setFlashLoading(false);
    }
  };

  const createFlashDay = async () => {
    if (!accessToken || !newDayDate || !newDayTitle.trim()) return;
    setNewDaySubmitting(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ event_date: newDayDate, title: newDayTitle.trim(), description: newDayDesc.trim() || undefined }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed');
      setFlashDays(prev => [data.flash_day, ...prev]);
      setShowNewDayForm(false);
      setNewDayDate(''); setNewDayTitle(''); setNewDayDesc('');
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to create flash day');
    } finally {
      setNewDaySubmitting(false);
    }
  };

  const deleteFlashDay = async (dayId: string) => {
    if (!accessToken || !confirm('Delete this flash day and all its designs?')) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash/${dayId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (!res.ok) throw new Error('Failed to delete');
      setFlashDays(prev => prev.filter(d => d.id !== dayId));
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to delete');
    }
  };

  const addFlashSlot = async (dayId: string) => {
    if (!accessToken || !newSlotTitle.trim() || !newSlotPrice) return;
    setNewSlotSubmitting(true);
    try {
      const form = new FormData();
      form.append('title', newSlotTitle.trim());
      form.append('price_pence', String(Math.round(parseFloat(newSlotPrice) * 100)));
      if (newSlotFile) form.append('image', newSlotFile);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash/${dayId}/slots`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accessToken}` },
        body: form,
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed');
      setFlashDays(prev => prev.map(d => d.id === dayId ? { ...d, slots: [...d.slots, data.slot] } : d));
      setAddingSlotFor(null);
      setNewSlotTitle(''); setNewSlotPrice(''); setNewSlotFile(null);
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to add slot');
    } finally {
      setNewSlotSubmitting(false);
    }
  };

  const deleteFlashSlot = async (dayId: string, slotId: string) => {
    if (!accessToken) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash/${dayId}/slots/${slotId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (!res.ok) throw new Error('Failed');
      setFlashDays(prev => prev.map(d => d.id === dayId ? { ...d, slots: d.slots.filter(s => s.id !== slotId) } : d));
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to delete slot');
    }
  };

  const releaseFlashSlot = async (dayId: string, slotId: string) => {
    if (!accessToken || !confirm('Release this claim and make the design available again?')) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/artist/flash/${dayId}/slots/${slotId}/release`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (!res.ok) throw new Error('Failed');
      setFlashDays(prev => prev.map(d => d.id === dayId ? {
        ...d,
        slots: d.slots.map(s => s.id === slotId ? { ...s, is_available: true, claimed_by_name: null, claimed_by_email: null, claimed_by_phone: null, claimed_at: null } : s),
      } : d));
    } catch (e) {
      setFlashError(e instanceof Error ? e.message : 'Failed to release');
    }
  };
  ...
  return (
    <div className="min-h-[100dvh]" style={{ background: 'var(--bg)' }}>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        ...
        {/* Flash Days tab */}
        {tab === 'flash' && (() => {
          const fmtFlashDate = (dateStr: string) => {
          ...
          return (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                <div>
                  <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontWeight: 300, fontSize: '1.875rem', color: 'var(--cream)', margin: 0, lineHeight: 1 }}>Flash Days</h2>
                  <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-low)', margin: '0.375rem 0 0' }}>
                    Pre-drawn designs · Fixed pricing · First come first served
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => { setShowNewDayForm(s => !s); }}
                  className="btn-primary"
                  style={{ fontSize: '0.8125rem', padding: '0.625rem 1.125rem' }}
                >
                  <span>{showNewDayForm ? 'Cancel' : '+ New Flash Day'}</span>
                </button>
              </div>

              {/* New Flash Day form */}
              {showNewDayForm && (
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border-light)', borderRadius: '0.75rem', padding: '1.5rem', marginBottom: '2rem' }}>
                  <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.6)', margin: '0 0 1.25rem' }}>New Flash Day</p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
                    <div>
                      <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.35rem' }}>Date *</label>
                      <input type="date" value={newDayDate} onChange={e => setNewDayDate(e.target.value)}
                        style={{ width: '100%', padding: '0.625rem 0.875rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.25rem', color: 'var(--cream)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.875rem', boxSizing: 'border-box' as const }}
                      />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.35rem' }}>Title *</label>
                      <input type="text" value={newDayTitle} onChange={e => setNewDayTitle(e.target.value)} placeholder="e.g. Spring Flash Day"
                        style={{ width: '100%', padding: '0.625rem 0.875rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.25rem', color: 'var(--cream)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.875rem', boxSizing: 'border-box' as const }}
                      />
                    </div>
                  </div>
                  <div style={{ marginBottom: '1.25rem' }}>
                    <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.35rem' }}>Description (optional)</label>
                    <textarea value={newDayDesc} onChange={e => setNewDayDesc(e.target.value)} rows={2} placeholder="Tell clients what to expect..."
                      style={{ width: '100%', padding: '0.625rem 0.875rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.25rem', color: 'var(--cream)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.875rem', resize: 'vertical' as const, boxSizing: 'border-box' as const }}
                    />
                  </div>
                  <button
                    type="button"
                    onClick={createFlashDay}
                    disabled={newDaySubmitting || !newDayDate || !newDayTitle.trim()}
                    className="btn-primary"
                    style={{ fontSize: '0.8125rem', opacity: newDaySubmitting ? 0.7 : 1 }}
                  >
                    <span>{newDaySubmitting ? 'Creating…' : 'Create Flash Day'}</span>
                  </button>
                </div>
              )}
              ...
              {/* Error */}
              {flashError && (
                <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.72rem', color: '#e57373', letterSpacing: '0.05em', marginBottom: '1rem' }}>{flashError}</p>
              )}

              {/* Loading */}
              {flashLoading && (
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '1rem' }}>
                  {[1, 2].map(i => <div key={i} className="skeleton" style={{ height: '120px', borderRadius: '0.75rem' }} />)}
                </div>
              )}

              {/* Empty state */}
              {!flashLoading && flashDays.length === 0 && (
                <div style={{ padding: '3rem', textAlign: 'center', border: '1px solid var(--border)', borderRadius: '0.75rem' }}>
                  <p style={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontSize: '1.5rem', color: 'rgba(201,168,76,0.2)', margin: '0 0 0.5rem' }}>No flash days yet</p>
                  <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', letterSpacing: '0.1em', color: 'var(--text-low)', margin: 0 }}>Create your first flash day above to get started.</p>
                </div>
              )}

              {/* Flash day list */}
              {!flashLoading && flashDays.map(day => {
                const past = isPast(day.event_date);
                const claimed = day.slots.filter(s => !s.is_available).length;
                const total = day.slots.length;
                const isAddingSlot = addingSlotFor === day.id;

                return (
                  <div key={day.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0.75rem', marginBottom: '1.5rem', overflow: 'hidden', opacity: past ? 0.65 : 1 }}>
                    {/* Day header */}
                    <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.75rem' }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
                          <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.55)', margin: 0 }}>
                            {fmtFlashDate(day.event_date)}
                          </p>
                          {past && (
                            <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.15rem 0.5rem', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: '2rem', color: 'var(--text-low)' }}>Past</span>
                          )}
                          {!past && !day.is_active && (
                            <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.15rem 0.5rem', background: 'rgba(255,100,100,0.08)', border: '1px solid rgba(255,100,100,0.15)', borderRadius: '2rem', color: 'rgba(255,120,120,0.7)' }}>Hidden</span>
                          )}
                        </div>
                        <h3 style={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontWeight: 300, fontSize: '1.5rem', color: 'var(--cream)', margin: 0, lineHeight: 1.1 }}>
                          {day.title}
                        </h3>
                        {day.description && (
                          <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.8125rem', color: 'var(--text-mid)', margin: '0.375rem 0 0', lineHeight: 1.6 }}>{day.description}</p>
                        )}
                      </div>
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexShrink: 0 }}>
                        {total > 0 && (
                          <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.1em', color: 'var(--text-mid)' }}>
                        ...
                        {!past && (
                          <button
                            type="button"
                            onClick={() => { setAddingSlotFor(isAddingSlot ? null : day.id); setNewSlotTitle(''); setNewSlotPrice(''); setNewSlotFile(null); }}
                            style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.375rem 0.75rem', background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '0.25rem', color: 'rgba(201,168,76,0.8)', cursor: 'pointer' }}
                          >
                            {isAddingSlot ? 'Cancel' : '+ Add Design'}
                          </button>
                        )}
                        <button
                          type="button"
                          onClick={() => deleteFlashDay(day.id)}
                          style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.375rem 0.75rem', background: 'rgba(255,80,80,0.06)', border: '1px solid rgba(255,80,80,0.12)', borderRadius: '0.25rem', color: 'rgba(255,100,100,0.6)', cursor: 'pointer' }}
                        >
                          Delete Day
                        </button>
                      </div>
                    ...
                    {/* Add slot form */}
                    {isAddingSlot && (
                      <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', background: 'rgba(201,168,76,0.03)' }}>
                        <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.5)', margin: '0 0 1rem' }}>New design</p>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.875rem', marginBottom: '0.875rem' }}>
                          <div>
                            <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.63rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.3rem' }}>Design name *</label>
                            <input type="text" value={newSlotTitle} onChange={e => setNewSlotTitle(e.target.value)} placeholder="e.g. Neo-trad butterfly"
                              style={{ width: '100%', padding: '0.5625rem 0.75rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.25rem', color: 'var(--cream)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.8125rem', boxSizing: 'border-box' as const }}
                            />
                          </div>
                          <div>
                            <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.63rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.3rem' }}>Price (£) *</label>
                            <input type="number" value={newSlotPrice} onChange={e => setNewSlotPrice(e.target.value)} placeholder="150" min="0" step="0.01"
                              style={{ width: '100%', padding: '0.5625rem 0.75rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.25rem', color: 'var(--cream)', fontFamily: '"DM Sans", sans-serif', fontSize: '0.8125rem', boxSizing: 'border-box' as const }}
                            />
                          </div>
                          <div>
                            <label style={{ display: 'block', fontFamily: '"DM Mono", monospace', fontSize: '0.63rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-low)', marginBottom: '0.3rem' }}>Photo (optional)</label>
                            <input type="file" accept="image/*"
                              onChange={e => setNewSlotFile(e.target.files?.[0] || null)}
                              style={{ width: '100%', padding: '0.4375rem 0', color: 'var(--text-mid)', fontFamily: '"DM Mono", monospace', fontSize: '0.68rem' }}
                            />
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => addFlashSlot(day.id)}
                          disabled={newSlotSubmitting || !newSlotTitle.trim() || !newSlotPrice}
                          className="btn-primary"
                          style={{ fontSize: '0.8rem', padding: '0.5625rem 1rem', opacity: newSlotSubmitting ? 0.7 : 1 }}
                        >
                          <span>{newSlotSubmitting ? 'Adding…' : 'Add Design'}</span>
                        </button>
                      </div>
                    )}
                    ...
                    {/* Slots */}
                    {day.slots.length === 0 ? (
                      <p style={{ padding: '1.25rem 1.5rem', fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', letterSpacing: '0.1em', color: 'var(--text-low)', margin: 0 }}>
                      ...
                      <div style={{
                        ...
                        {day.slots.map(slot => (
                          <div key={slot.id} style={{ background: 'var(--surface)', padding: '1.125rem' }}>
                            {/* Image thumbnail */}
                            {slot.image_url && (
                              <div style={{ aspectRatio: '1', marginBottom: '0.75rem', borderRadius: '0.25rem', overflow: 'hidden', background: 'var(--bg)' }}>
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={slot.image_url} alt={slot.title} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: slot.is_available ? 'none' : 'grayscale(50%) brightness(0.6)' }} />
                              </div>
                            )}
                            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.8125rem', fontWeight: 500, color: 'var(--cream)', margin: '0 0 0.1875rem', lineHeight: 1.3 }}>{slot.title}</p>
                            <p style={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontSize: '1.125rem', color: 'var(--gold)', margin: '0 0 0.625rem', lineHeight: 1 }}>
                              £{Math.round(slot.price_pence / 100)}
                            </p>
                            {slot.is_available ? (
                              <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(100,200,120,0.7)', padding: '0.2rem 0.5rem', background: 'rgba(100,200,120,0.07)', border: '1px solid rgba(100,200,120,0.15)', borderRadius: '2rem' }}>
                                Available
                              </span>
                            ) : (
                              <div>
                                <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,150,150,0.7)', padding: '0.2rem 0.5rem', background: 'rgba(255,100,100,0.07)', border: '1px solid rgba(255,100,100,0.15)', borderRadius: '2rem', display: 'inline-block', marginBottom: '0.375rem' }}>
                                  Claimed
                                </span>
                                {slot.claimed_by_name && (
                                  <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.63rem', letterSpacing: '0.08em', color: 'var(--text-mid)', margin: '0.25rem 0 0', lineHeight: 1.5 }}>
                                    {slot.claimed_by_name}<br />
                                    {slot.claimed_by_email && <span style={{ color: 'var(--text-low)' }}>{slot.claimed_by_email}</span>}
                                    {slot.claimed_by_phone && <><br /><span style={{ color: 'var(--text-low)' }}>{slot.claimed_by_phone}</span></>}
                                  </p>
                                )}
                              </div>
                            )}
                            <div style={{ display: 'flex', gap: '0.375rem', marginTop: '0.75rem', flexWrap: 'wrap' }}>
                              {!slot.is_available && (
                                <button
                                  type="button"
                                  onClick={() => releaseFlashSlot(day.id, slot.id)}
                                  style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.25rem 0.5rem', background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '0.25rem', color: 'rgba(201,168,76,0.6)', cursor: 'pointer' }}
                                >
                                  Release
                                </button>
                              )}
                              <button
                                type="button"
                                onClick={() => deleteFlashSlot(day.id, slot.id)}
                                style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.25rem 0.5rem', background: 'rgba(255,80,80,0.05)', border: '1px solid rgba(255,80,80,0.1)', borderRadius: '0.25rem', color: 'rgba(255,100,100,0.55)', cursor: 'pointer' }}
                              >
                                Delete
                              </button>
                            </div>
